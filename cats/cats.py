...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    if valid_words.count(typed_word)>0:
        return typed_word
    mn=limit+100
    for x in valid_words:
        if diff_function(typed_word,x,limit)<mn:
            mn=diff_function(typed_word,x,limit)
            rec=x
    if mn<=limit:
        return rec
    else:
...
def minimum_mewtations(start, goal, limit):
    ...
    if limit<0:  # Fill in the condition
        ...
        add = 1+min(minimum_mewtations(start,goal[1:],limit-1),minimum_mewtations(start[1:],goal,limit-1))  # Fill in these lines
        remove = add
...
def time_per_word(words, times_per_player):
    ...
    for i in times_per_player:
        l1=len(i)
        tmp=[i[x]-i[x-1] for x in range(1,l1)]
        times.append(tmp)
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if time(match,j,i)<time(match,rec,i):
                rec=j
        ans[rec].append(word_at(match,i))
    return ans
    # END PROBLEM 10
