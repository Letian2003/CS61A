...
def merge(lst1, lst2):
    ...
    if lst1[0]<lst2[0]:
        s=[lst1[0]]
        s.extend(merge(lst1[1:],lst2))
        return s 
    s=[lst2[0]]
    s.extend(merge(lst1,lst2[1:]))
    return s
...
def remove_odd_indices(lst, odd):
    ...
    if odd is True:
        s=[lst[0]]
        s.extend(remove_odd_indices(lst[1:],not odd))
        return s
    else:
...
class SmartFridge:
    ...
    def add_item(self, item, quantity):
        if self.items.get(item,0)==0:
            ...
            self.items[item]+=quantity
        print("'I now have",self.items[item],item+"'")

    def use_item(self, item, quantity):
        if self.items[item]>quantity:
            self.items[item]-=quantity
            print("'I have",self.items[item],item,"left'")
        else:
            self.items[item]=0
